...
printf("escolha os pacotes de viagem a seguir(A B ou C MAISCULO):\n");
printf("A)Sao Paulo R$1300,00\nB)Rio De Janeiro R$ 2000,00\nC)Brasilia R$1700,00\n");
scanf("%c",&pacotes);

printf("segue os descontos relativos a quantidade de pessoas(A B ou C MAISCULO): ");
printf("\n");
printf("A)MAX 5 PESSOAS = 15 por cento\nB)MAX 10 PESSOAS = 25 por cento\nC)MAX 15 PESSOAS = 35 por cento\n");
printf("\n");
scanf("%d",&qnt);	
	
if (qnt<=5 && pacotes=='A') {
    valor = 1300*0.85;
    printf("valor final: R$%.2f cada",valor);	
